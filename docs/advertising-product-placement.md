...
higher-confidence differentiators in the blueprint (§6: medium-high) because
...
- **Creator consent is required, not implied by popularity.** A title
  ...
  preference), and shares in placement revenue (see `tokenomics.md` §3).
...
Phase 1/2 requirement (`architecture.md` §5).
...
- Placements are labeled as paid placement in metadata (extends the
  ...
  `content-quality-gates.md` §4), consistent with standard advertising-
...
- Regional advertising-disclosure and endorsement regulations vary; not
  enumerated here — flagged consistent with `ASSUMPTIONS_AND_RISK.md` §5 as
...
signal used elsewhere (`tokenomics.md` §5), specifically so that gaming
