...
Read `ASSUMPTIONS_AND_RISK.md` §3 and §5 first. This document does not
...
| Option | Pros | Cons |
|---|---|---|
| Public blockchain token | Transferable, tradable, aligns with "cryptocurrency" as stated in the brief; composable with external crypto ecosystem | Highest regulatory exposure (§5 of risk doc); volatile; gas/settlement cost; hardest to reverse fraud/abuse |
...
Recommendation for Phase 1/2 piloting (`architecture.md` §5): start with
...
`ASSUMPTIONS_AND_RISK.md` §4 (paid vs. unpaid control cohort convergence
test) — because the historical base rate in §3 says this is the version
...
staking (`p2p-rendering.md` §6) — one investment in anti-Sybil
...
- Proof-of-personhood at account creation (mechanism unspecified — a
  ...
  `ASSUMPTIONS_AND_RISK.md` §2.1/§3, not named here to avoid asserting a
