...
| Quality Gate Pipeline | Multi-stage review: automated compliance checks -> automated technical QC -> human curator review -> publish decision. See `content-quality-gates.md`. | Automated creative-quality gating is low-confidence without human review (§2.2, §6) |
...
| Node Registry & Staking | Onboards compute contributors, tracks stake/reputation, assigns jobs, slashes for bad output | P2P viability unproven for this workload (§2.3) |
...
| Rating & Leaderboard Service | Collects ratings/downvotes, computes leaderboard, feeds Wallet for payout | Pay-per-rating signal integrity is the platform's single biggest open risk (§3) |
| Identity & Reputation Service | Proof-of-personhood, account reputation, Sybil-resistance signal shared by Rating and Node Registry | Anti-Sybil infra is expensive and imperfect (§3, H1) |
| Wallet / Ledger Service | Token balances, payouts to raters/creators/node operators, advertiser billing, fiat on/off-ramp integration | Legal/regulatory questions open (§5) |
...
- Cloud burst capacity exists in the architecture from day one, not as a
  ...
  `ASSUMPTIONS_AND_RISK.md` §3, the platform should be able to run its core
...
in `ASSUMPTIONS_AND_RISK.md` §4 — a phase does not proceed on schedule, it
